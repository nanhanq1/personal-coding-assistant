...
核心原则：中文讲解；先直觉再原理再源码再工程实践；先讲调用链、目标文件、输入输出、计划测试、验收标准和安全边界；用户先写代码则先评审、补中文解释性注释，再给参考实现对比；每次只聚焦一个模块。

详细教学流程见 `docs/16_TEACHING_WORKFLOW.md`。该文件只保存稳定教学流程，不保存当前周次或任务状态。
...
- `docs/15_MEMORY_SYSTEM.md`：仓库协作记忆、Codex 外部记忆和未来运行时记忆边界
- `docs/16_TEACHING_WORKFLOW.md`：教学流程和每日收口门禁
...
**开始时**：按 `AGENTS.md` 读取必读文件 → 以 `docs/09_NEXT_ACTIONS.md` 确认真正进度 → 给出今日任务、调用链、目标文件、输入输出、测试和边界 → 实现核心代码或文档 → 更新测试和文档

**结束时**：输出完成内容、修改文件、验证结果、面试题状态、已更新的项目记忆文件和 `Next Required Input`。面试题未回答时不归档。
