...
维护说明（2026-07-12）：已完成审计 F-01 的 P0 shell wrapper 最小 ASK 修复与回归验证；这是进入 Day 1 前的安全维护，不表示 Repo Scanner 已开始。

维护说明（2026-07-12）：已完成审计 F-02/F-03 的 P1 稳定错误修复；非法工具名保持在 `ToolResult` 信封内，approval 字段和时间采用严格契约。Repo Scanner 仍未开始。
